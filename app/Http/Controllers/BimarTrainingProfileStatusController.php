...
class BimarTrainingProfileStatusController extends Controller
{
    ...
    {    if (Auth::guard('administrator')->check() || Auth::guard('operation_user')->check() || Auth::guard('trainer')->check()) {
        $data = Bimar_Training_Profile_Status::all();
        return view('admin.training_profile_status',compact('data'));
    }else{
    ...
    {   if (Auth::guard('administrator')->check() || Auth::guard('operation_user')->check() || Auth::guard('trainer')->check()) {
        return view('admin.training_profile_status');
    }else{
        return redirect()->route('home');
    }
    }
     ...
     */
    public function show(Bimar_Training_Profile_Status $Bimar_Training_Profile_Status)
    {
        //
    }
     ...
     */
    public function destroy(Bimar_Training_Profile_Status $Bimar_Training_Profile_Status)
    {
        //
    }

    public function updateSwitch($profileId)
    {    if (Auth::guard('administrator')->check() || Auth::guard('operation_user')->check() || Auth::guard('trainer')->check()) {
        $profile = Bimar_Training_Profile_Status::find($profileId);
        if($profile){
            if($profile->tr_profile_status){
                $profile->tr_profile_status =0;
            }
            else{
                $profile->tr_profile_status =1;
            }
            $profile->save();
        }
        return back();
